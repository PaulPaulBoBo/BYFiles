//
//  BYFindUnusedImportWindowController.m
//  BYFiles
//
...
@interface BYFindUnusedImportWindowController ()

@property (weak) IBOutlet NSButton *chooseClasses;

@end

@implementation BYFindUnusedImportWindowController
...
- (void)setupTool {
    if(self.tool == nil) {
        self.tool = [[FindUnusedImport alloc] init];
...
-(void)clickChooseClasses {
    ...
    [ChoosePathTool openPanelCanChooseFiles:NO canMultipleSelection:NO finish:^(NSArray * _Nonnull paths) {
        ...
        [((FindUnusedImport*)strongSelf.tool) configOrderClassPaths:pathTool.files];
    ...
    ((FindUnusedImport*)self.tool).findCompletion = ^(NSArray * _Nonnull paths) {
...
-(void)choosePathFinish {
    [((FindUnusedImport *)self.tool) analyzeFolder];
}
