//
//  ModulesWindowController.m
//  BYFiles
//
//  Created by Liu on 2023/3/16.
//

#import "ModulesWindowController.h"
#import "Masonry.h"

@interface ModulesWindowController ()<NSSplitViewDelegate>
...
@implementation ModulesWindowController

- (void)windowDidLoad {
    [super windowDidLoad];
    [self.contentViewController addChildViewController:self.splitVC];
    [self.contentViewController.view addSubview:self.splitVC.view];
    [self.splitVC.view mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.contentViewController.view);
    }];
}
...
- (NSSplitViewController *)splitVC {
    if(_splitVC == nil) {
        ...
        [_splitVC addSplitViewItem:self.leftItem];
        [_splitVC addSplitViewItem:self.rightItem];
    }
    return _splitVC;
}
